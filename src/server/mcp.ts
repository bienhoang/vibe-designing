...
let activePort: number = parseInt(process.env.VIBE_DESIGNING_PORT || "3055");
...
function sendCommandToFigma(
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = {
      ...
      ...(command === "join"
        ? { channel: (params as any).channel, role: "mcp", version: VIBE_DESIGNING_VERSION, name: basename(process.cwd()) }
    ...
    const timeout = setTimeout(() => {
      if (pendingRequests.has(id)) {
        ...
        reject(new Error(
          ...
          `Check the Vibe Designing plugin window: the port and channel name must match.`
        ));
      }
    }, timeoutMs);
    ...
    try {
      ...
      return {
        content: [{
          type: "text",
          text: connected
            ? `Tunnel reset: ${body.message}. Reconnected on port ${activePort}.\n\nIMPORTANT: The Figma plugin was also disconnected. Ask the user to reopen the Vibe Designing plugin in Figma (or click "Reset tunnel" in the plugin panel). Then call \`join_channel\` followed by \`ping\`.`
            : `Tunnel reset: ${body.message}. Reconnection in progress.\n\nIMPORTANT: The Figma plugin was also disconnected. Ask the user to reopen the Vibe Designing plugin in Figma (or click "Reset tunnel" in the plugin panel). Then call \`join_channel\` to retry.`,
        }],
      };
    } catch (error) {
      return {
